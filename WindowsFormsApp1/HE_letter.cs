...
namespace WindowsFormsApp1
{
    public partial class HE_letter : Form
    {
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            string database = "";
            string database2 = "";
            string cls = Microsoft.VisualBasic.Interaction.InputBox("ENTER CLASS FOR WHICH LETTER HAS TO BE GENERATED", "INPUT CLASS", "10");
            if (cls == "10")
            {
                database = "EXM10C";
                database2 = "HE10C";
            ...
            while (dr2.Read())
            {
                ...
                PdfWriter pwriter = PdfWriter.GetInstance(pdoc, new FileStream("D:\\ABHISHEK\\WORK2022C\\PDF LETTERS\\HELETTER\\" + dr2["sch_no"].ToString() + "_" + dr2["HENO"].ToString() + "_HE2022.pdf", FileMode.Create));
                ...
                footer.SetAbsolutePosition(15, 8);
                footer.Alignment = 1;
                rosign.ScaleToFit(90f, 30f);
                rosign.SetAbsolutePosition(470, 210);
                pdoc.Open();
                ...
                pdoc.Add(p3);
                Paragraph p4 = new Paragraph(str: "No.:CBSE/RO-PTN/EXAM/SPOT/COMP 2022/" + dr2["hesub"].ToString() + " - " + dr2["heno"].ToString() + "/REVISED                         Date:" + date_str) { Alignment = Element.ALIGN_LEFT };
                pdoc.Add(p4);
                Paragraph p34 = new Paragraph(str: "He No." + dr2["heno"].ToString(),hbold) { Alignment = Element.ALIGN_RIGHT };
                pdoc.Add(p34);
                Paragraph p5 = new Paragraph(str: dr2["HENAME"].ToString().ToUpper() + ", " + dr2["POST"].ToString().ToUpper() + "\n" + dr2["headd1"].ToString().ToUpper() + "(School No.: "+ dr2["sch_no"].ToString() + ")\n" + dr2["headd2"].ToString().ToUpper() + "\n" + dr2["headd3"].ToString().ToUpper() + "\n" + dr2["headd4"].ToString().ToUpper() + ", " + dr2["headd5"].ToString().ToUpper() + " - " + dr2["hepin"].ToString()) { Alignment = Element.ALIGN_LEFT };
                pdoc.Add(p5);
                Paragraph p6 = new Paragraph(str: "\nSUB.:  INTIMATION LETTER WITH EXAMINERS LIST FOR SPOT EVALUATION FOR HEAD EXAMINER OF SUBJECT " + dr2["subname"].ToString() + "(" + dr2["hesub"].ToString() + ") OF CLASS " + dr2["heclass"].ToString() + " Compartment Exam 2022") { Alignment = Element.ALIGN_LEFT };
                pdoc.Add(p6);
                Paragraph p7 = new Paragraph(str: "\nSir/Madam,") { Alignment = Element.ALIGN_LEFT };
                pdoc.Add(p7);
                Paragraph p8 = new Paragraph(str: "      This is in  continuation  to  this  office  letter  no. RO(PTN)/CONF./H.E./" + dr2["hesub"].ToString() + "/" + dr2["HENO"].ToString() + "/COMP 2022   and  subsequently  your  acceptence for appointment/assignment of Head Examiner in the subject " + dr2["subname"].ToString().ToUpper() + "(" + dr2["hesub"].ToString() + "),\n") { Alignment = Element.ALIGN_JUSTIFIED };
                pdoc.Add(p8);
                Paragraph p38 = new Paragraph(str: "\n      In this connection, I am to inform you that the Evaluation in the subject  " + dr2["subname"].ToString().ToUpper() + "(" + dr2["hesub"].ToString() + ") is scheduled to   commence  on dt. " + dr2["SDATE"].ToString() + " at your Centre and the work shall have to  be  completed  before dt. " + dr2["EDATE"].ToString() + "  positively.",hbold) { Alignment = Element.ALIGN_JUSTIFIED };
                pdoc.Add(p38);
                Paragraph p9 = new Paragraph(str: "\n      You must be aware that the Evaluation Work is very sensitive in nature and the future of the students is going to be  affected by the  manner in which the Evaluation is done.  Therefore, it is requested that due care, attention and complete  dedication should be given to  the  Evaluation  of  Answer  Books  by ensuring accuracy & uniformity as per  Marking  Scheme  and performance.  Also it may be noted that  the  work  assigned  to  you is strictly time  bound & confidential and the entire Post - Examination work will have a direct bearing for timely completion of Evaluation Work and subsequently scheduled declaration  of result.  You are therefore requested to complete the task including coordination work within prescirbed period i.e. 03-04 days time(maximum) positively.\n\n      Though  the  examiners  have  been  appointed  by the Regional Office, Patna, CBSE.  However, Head Examiner can make alternate arrangements in consultation with the CNS when  the  requisite no. of  examiners appointed by the Board does not report for evaluation with a valid reason.") { Alignment = Element.ALIGN_JUSTIFIED };
                pdoc.Add(p9);
                Paragraph p10 = new Paragraph(str: "\n      The Head  Examiners  will have to ensure that  such appointment is made strictly as per the qualifications of examiners given under rule 55(ii) of exam by  laws  and as per  guidelines  of  Spot Evaluation Compt. Exam 2022. However, details of the appointments made at local level be sent  to  the  Regional  Office, Patna, CBSE  for necessary action/record and updations.") { Alignment = Element.ALIGN_JUSTIFIED };
                pdoc.Add(p10);
                pdoc.NewPage();
                pdoc.Add(header); //Adding Header
                pdoc.Add(footer); //Adding Foter
