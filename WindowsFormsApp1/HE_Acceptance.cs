    ...
    public partial class HE_Acceptance : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            while (dr.Read())
            {
                ...
                PdfWriter pwriter = PdfWriter.GetInstance(pdoc, new FileStream("D:\\ABHISHEK\\WORK2022C\\PDF LETTERS\\HE\\" + dr["he_sch"].ToString() + "_" + dr["heno"].ToString() + ".pdf", FileMode.Create));
                ...
                rosign.SetAbsolutePosition(470, 130);
                pdoc.Open();
                ...
                pdoc.Add(p3);
                Paragraph p4 = new Paragraph(str: "No.:CBSE/RO(PTN)/CONF./H.E./" + dr["hesub"].ToString() + "/" + dr["heno"].ToString() + "/COMP-2022/                                   Dated:" + date_str+"\n\n") { Alignment = Element.ALIGN_LEFT };
                pdoc.Add(p4);
                Paragraph p5 = new Paragraph(str: dr["hename"].ToString().ToUpper() + ",\n"+ dr["he_desig"].ToString() + "(School no.: " + dr["he_sch"].ToString() + ")\n"  + dr["headd1"].ToString() + "\n" + dr["headd2"].ToString() + "\n" + dr["headd3"].ToString() + "\n" + dr["headd4"].ToString() + " " + dr["headd5"].ToString() + " - " + dr["hepin"].ToString() + "\n\n") { Alignment = Element.ALIGN_LEFT };
                pdoc.Add(p5);
                Paragraph p6 = new Paragraph(str: "Sub:  Appointment Cum  Acceptance Letter Of Head Examiner For Class " + dr["HECLASS"].ToString() + " SUBJECT (" + dr["hesub"].ToString() + dr["hesub2"].ToString() + ")-(" + dr["subname"].ToString() + dr["subname2"].ToString() + ") for Compartment Examination - 2022.\n\nSir/Madam,\n\n") { Alignment = Element.ALIGN_LEFT };
                pdoc.Add(p6);
                ...
                Paragraph p7 = new Paragraph(str: "      The Board is pleased to  appoint  you as Head  Examiner for the  subject  " + dr["subname"].ToString() + dr["subname2"].ToString() + "(" + dr["hesub"].ToString() + dr["hesub2"].ToString() + ") of Class " + dr["HECLASS"].ToString() + " Compartment Examination - 2022 to be held from " + d1 + " to " + d2 + ". \n\n") { Alignment = Element.ALIGN_JUSTIFIED };
                pdoc.Add(p7);
                Paragraph p8 = new Paragraph(str: "      In order to ensure proper manageability, effective supervision and timely completion of evaluation work with accuracy, the Board is also appointing Chief Nodal Supervisor (CNS) at selected cities.\n\n      The Chief Nodal Supervisor would be a Principal of the School and under his / her supervision 3 to\r\n10 Vice Principals / PGT(s) will be appointed as Head  Examiner.  The   Head  Examiner  would  do the evaluation work in the school of Chief  Nodal Supervisor  and the  time of evaluation will  be decided  by you  in consultation with the Chief Nodal Supervisor as per schedule.\n\n      The tentative  date  for  commencement of  Spot Evaluation in the  subject is after  05 - 06 days from the date of conduct of Examination.  The Answer Books will be handed over  to  you for  evaluation by Chief Nodal Supervisor.\n\n") { Alignment = Element.ALIGN_JUSTIFIED };
                pdoc.Add(p8);
                Paragraph p9 = new Paragraph(str: "The details of your Nodal Centre where the Evaluation will be done are as under :\n\n",bold) { Alignment = Element.ALIGN_LEFT };
                pdoc.Add(p9);
                PdfPTable tbl = new PdfPTable(1);
                tbl.HorizontalAlignment = 1;
                tbl.AddCell(new Phrase(str: "SCHOOL NO.: "+dr["cns_schno"].ToString() + "\nSCHOOL NAME: " + dr["cnsadd1"].ToString() + "\nSCHOOL ADDRESS: " + dr["cnsadd2"].ToString() + "\n" + dr["cnsadd3"].ToString() + "\n" + dr["cnsadd4"].ToString() + " " + dr["cnsadd5"].ToString() + " - " + dr["cnspin"].ToString() + "\nCNS MOBILE NO.: " + dr["cnsmobile"].ToString()));
                pdoc.Add(tbl);
                pdoc.NewPage();
                pdoc.Add(header); //Adding Header
                pdoc.Add(footer); //Adding Foter
                ...
                pdoc.Add(p11);
                Paragraph p12 = new Paragraph(str: "      Further, as per past experience it has been observed that quality  of evaluation done at some of spot evaluation centres was  not found satisfactory and also not upto  the  desired  level.  A large number of mistake cases were detected  during  the  course  of  scrutiny on account of  Re-Verification/Re-evaluation  work that shows the evaluation were not  done  properly & raised  questions  on   the credibility on working  of  Head  Examiners and  examiners  who  participated in the evaluation work.\n\n", bold) { Alignment = Element.ALIGN_JUSTIFIED };
                pdoc.Add(p12);
                Paragraph p101 = new Paragraph(str: "      As you are aware  that  the  students  look  at  these examination as a final evaluation of their academic performance.  The competent authority of the board has taken it seriously owing to large no. of  mistakes during   evaluation.\n\n", bold) { Alignment = Element.ALIGN_JUSTIFIED };
                pdoc.Add(p101);
                Paragraph p41 = new Paragraph(str: "      Also  from  exam  2012  the   students / examinees   may take  photocopy of their answer sheets under rti act 2005 as per orders of the hon'ble supreme court of india. As  from 2014 exam candidate  can  get  re-evaluated their answer books on certain subjects.\n\n") { Alignment = Element.ALIGN_JUSTIFIED };
                pdoc.Add(p41);
                Paragraph p40 = new Paragraph(str: "      Therefore, it is requested that proper attention towards evaluation should be given and answer books  of the subject be evaluated in perfect manner strictly in accordance with the marking scheme and posting marks carefully in Answer Books. \n", bold) { Alignment = Element.ALIGN_JUSTIFIED };
                pdoc.Add(p40);
                pdoc.NewPage();
                pdoc.Add(header); //Adding Header
                pdoc.Add(footer); //Adding Foter
                ...
                pdoc.Add(p13);
                Paragraph p14 = new Paragraph(str: "\n      Further, Head  Examiner  should  check the all Answer Books before the start of work whether  any  Answer Books of Physically Challenged children (Spastic, Blind, Physically  Handicapped  and Dyslexic children) have been erroneously received along with the  Answer  Books of other candidates.  If the Answer Books of Physically Challenged children are found mixed with the Answer Books of other candidates, these be  immediately returned  to the undersigned unevaluated through Sealed/Insured Speed  Post Parcel in consultation with the Chief Nodal Supervisor.\n\n      After  Evaluation,  the  Answer  Books   will   be   serialized in ascending order and should be packed century wise in respective  Answer  Books bags.\n") { Alignment = Element.ALIGN_JUSTIFIED };
                pdoc.Add(p14);
                Paragraph p15 = new Paragraph(str: "\n      It may also be  noted  that  the  Evaluation  is the  most  important part  of the whole Examination system and it determines  the  future career of the  students.  Therefore, you are  requested  to  take every possible care to ensure objective and  judicious  Evaluation  to  safeguard the interest of the students and also to avoid any future complications / allegations.\n\n      A  copy of the guidelines for Spot Evaluation will be sent to you in due course for your reference and strict compliance. The Remunerations, Conveyance and Refreshment charges will be paid to you and sub - examiners as per spot Evaluation Guidelines Compartment Examination 2022 issued by the Board.\n\n") { Alignment = Element.ALIGN_JUSTIFIED };
                pdoc.Add(p15);
                Paragraph p16 = new Paragraph(str: "      Your acceptance of the assignment being offered by the Board should reach  to the undersigned immediately in the enclosed proforma  duly  completed  in all  respects through  EMAIL to abcell.ropatna@cbseshiksha.in and ropatna.cbse@nic.in. For any assistance with regards to above: Kindly email to abcell.ropatna@cbseshiksha.in with complete detail.") { Alignment = Element.ALIGN_JUSTIFIED };
                pdoc.Add(p16);
                ...
                pdoc.Add(tbl3);
                Paragraph p24 = new Paragraph(str: "\nI undertake that information  mentioned above are true and there is no change in above records.\n\n") { Alignment = Element.ALIGN_CENTER };
                pdoc.Add(p24);
                Paragraph p23 = new Paragraph(str: "Yours faithfully,\n\nSignature ...................................\nName of HE: " + dr["hename"].ToString().ToUpper() + "\nHE No.: " + dr["heno"].ToString() + "\nSchool No.: " + dr["he_sch"].ToString()) { Alignment = Element.ALIGN_RIGHT };
                pdoc.Add(p23);
                //
                pdoc.Close();
            }
